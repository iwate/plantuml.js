const plantuml = (() => {
    const initialize = async (cheerpjPath = "/app/plantuml-wasm") => {
        await Promise.all([
            cheerpjInit({
                javaProperties: [`sun.java2d.fontpath=${cheerpjPath}/fonts`],
                preloadResources: _runtimeResources(cheerpjPath)
            }),
    ...
    const renderPng = (pumlContent) => {
        return new Promise((resolve, reject) => {
            ...
            cjCall("com.plantuml.wasm.v1.Png", "convert", "light", `/files/result-${resultFileSuffix}.png`, _withDefaultFont(pumlContent)).then((result) => {
